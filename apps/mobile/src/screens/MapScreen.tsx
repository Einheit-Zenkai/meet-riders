...
const MapScreen = ({ navigation }: MapScreenProps): JSX.Element => {
  ...
  useEffect(() => {
    ...
    const init = async (): Promise<void> => {
      if (isWeb) {
        // react-native-maps is not supported reliably on web in this project.
        if (active) {
          setLocating(false);
        }
        return;
    ...
    return (
      <View style={styles.root}>
        ...
        <View style={styles.webNotice}>
          ...
          <Text style={styles.webSubtitle}>Open the app on iOS/Android (Expo Go) to use pin drop.</Text>
        </View>
      </View>
    );
  }
