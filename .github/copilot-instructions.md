## Overview
- Meet-Riders runs as a pnpm workspace with the Next.js web client in `apps/web` (Next.js 15 + React 19) and the Expo-powered mobile app in `apps/mobile`. Both target the same Supabase backend for auth, storage, and RPC.
- Web UX divides unauthenticated flows under `apps/web/src/app/(Authentication)` and the signed-in product under `apps/web/src/app/(main)` with shared scaffolding in `MainLayout` and the global providers in `apps/web/src/app/layout.tsx`.
...
- Middleware in `apps/web/middleware.ts` refreshes Supabase sessions and enforces route access; respect its protected/public route lists when adding pages.
- When touching dashboard data pipelines, reuse `useDashboard`, `useDashboardDataStore`, and `useDashboardParties` from `apps/web/src/app/(main)/dashboard`; they already enforce onboarding, periodic refresh, friend-only visibility, and party ordering.
...
- Mobile UI currently focuses on the login flow with React Navigation stack screens defined in `apps/mobile/src/navigation/AppNavigator.tsx`; keep initial iterations light (buttons, layout) before wiring Supabase auth.
...
- `StoreInitializer` in `apps/web/src/app/layout.tsx` primes auth and party cleanup; do not remove this from the root tree.
- API routes (e.g., `apps/web/src/app/api/delete-account/route.ts`) assume environment variables `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, and `SUPABASE_SERVICE_ROLE_KEY`; keep secrets server-side and require `Authorization: Bearer <access_token>`.


## Developer Workflow
- Install dependencies from the repo root via `pnpm install`; workspaces register everything under `apps/*` and `packages/*`.
- Web: `pnpm dev:web` (or `pnpm --filter web dev`) runs the Next.js app with Turbopack; `pnpm --filter web build` and `pnpm --filter web lint` cover CI steps.
- Mobile: `pnpm dev:mobile` (or `pnpm --filter mobile start`) launches the Expo dev server. Keep Expo-specific env vars in `apps/mobile/.env` using the `EXPO_PUBLIC_` prefix.
- Environment setup needs a Supabase project with tables from `backend/sql/*.sql`; keep web secrets in `apps/web/.env.local`.
...
## Conventions
- Type imports resolve via the `@/*` alias configured in `apps/web/tsconfig.json`; prefer colocated modules under `apps/web/src` to leverage it. Shared TypeScript helpers belong in `packages/utils` and can be imported via `@meetriders/utils`.
